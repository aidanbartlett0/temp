...
def runGame():
    ...
    while not FINISHED:
        ...
        move, currentState = moveAndState[:2]
        full_stats = (moveAndState[2:] + [0]*8)[:8]

        alpha_beta_cutoffs_this_turn, num_static_evals_this_turn, \
        zobrist_table_num_entries_this_turn, zobrist_table_num_hits_this_turn, \
        alpha_beta_cutoffs, num_static_evals, \
        zobrist_table_num_entries, zobrist_table_num_hits = full_stats


        moveReport = "Move is by "+who+" to "+str(move)
        moveReport += f"\nStatic evals: {num_static_evals_this_turn} \n ab cutoffs: {alpha_beta_cutoffs_this_turn} \
                     \n zobrist hits: {zobrist_table_num_hits_this_turn} \n zobrist writes: {zobrist_table_num_entries_this_turn}"
        ...
        if possibleWin != "No win":
            ...
            possibleWin += f"Static evals: {num_static_evals} \n ab cutoffs: {alpha_beta_cutoffs} \
                     \n zobrist hits: {zobrist_table_num_hits} \n zobrist writes: {zobrist_table_num_entries}"
    ...
    if USE_HTML: gameToHTML.stateToHTML(currentState)
    who = currentState.whose_move
    renderCommentary(f"Game Over; it's a draw. Static evals: {num_static_evals} \n ab cutoffs: {alpha_beta_cutoffs} \
                     \n zobrist hits: {zobrist_table_num_hits} \n zobrist writes: {zobrist_table_num_entries}")
    if USE_HTML: gameToHTML.reportResult(f"Game Over; it's a draw. Static evals: {num_static_evals} \n ab cutoffs: {alpha_beta_cutoffs} \
                     \n zobrist hits: {zobrist_table_num_hits} \n zobrist writes: {zobrist_table_num_entries}")
    if USE_HTML: gameToHTML.endHTML()
